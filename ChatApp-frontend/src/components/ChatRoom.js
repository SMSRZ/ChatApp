...
const ChatRoom = () => {
    ...
    const onConnected = () => {
        ...
        // stompClient.subscribe('/user/'+userData.username+'/active-user',onActiveUsersReceived);
...
stompClient.subscribe('/user/' + username + '/active-users', onActiveUsersReceived);
    ...
    stompClient.send("/app/adduser", {}, JSON.stringify(chatMessage)); // ✅ Send to adduser endpoint
};
    ...
    const onMessageReceived = (payload)=>{
        ...
        switch(payloadData.status){
            ...
            setPublicChats([...publicChats]);
             // Remove the user from privateChats (i.e. from the user list)
             if (privateChats.has(payloadData.senderName)) {
               privateChats.delete(payloadData.senderName);
                 setPrivateChats(new Map(privateChats));

                 // Optional: If you were viewing that chat, go back to main chatroom
      ...
      {publicChats.map((chat, index) => (
        ...
        >
          ...
          <div className="message-data">
            {chat.senderName === "System" ? (
              <i>{chat.message}</i> // style system messages as italic
            ) : (
              chat.message
            )}
          </div>
